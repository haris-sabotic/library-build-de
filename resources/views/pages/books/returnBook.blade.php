            ...
            @if(count($rentedBooks) > 0)
                    <div class="scroll height-dashboard px-[30px]">
                        <div class="flex items-center justify-between py-4 pt-[20px] space-x-3 rounded-lg">
                            ...
                            <form action="{{ route('searchReturn', ['book' => $book->id]) }}" method="GET">
                                <div class="flex items-center pl-6 py-4 space-x-3 rounded-lg ml-[292px]">
                                    <div class="flex items-center">
                                        <div class="relative text-gray-600 focus-within:text-gray-400">
                                            ...
                                            <input type="search" name="searchReturn"
                                    ...
                                    <button name="buttonSearchReturn"
                                    ...
                                    @foreach($rentedBooks as $rentedBook)
                                        <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                                            ...
                                            <td class="flex flex-row items-center px-2 py-4 xl:px-4 whitespace-nowrap">
                                                <img class="object-cover w-8 h-10 mr-2 rounded-full" src="/storage/image/{{$rentedBook->student->photo}}"
                                                    alt="" />
                                                <a href="{{route('studentProfile', ['user' => $rentedBook->student])}}">
                                                    <span class="text-xs font-medium text-center 2xl:text-sm">{{$rentedBook->student->name}}</span>
                                                </a>
                                            </td>
                                            <td class="px-2 py-4 text-xs leading-5 2xl:text-sm xl:px-4 whitespace-nowrap">{{$rentedBook->rent_date}}</td>
                                            <td class="px-2 py-4 text-xs leading-5 2xl:text-sm xl:px-4 truncate max-w-[150px] xl:max-w-full">{{ \Carbon\Carbon::parse($rentedBook->rent_date)->diffAsCarbonInterval() }}</td>
                                            <td class="px-2 py-4 leading-5 xl:px-4 whitespace-nowrap">
                                                    ...
                                                    @else
                                                        <span class="px-[6px] py-[2px] bg-green-200 text-green-800 rounded-[10px] text-xs 2xl:text-sm">
                                                            Nema prekoračenja
                                                        </span>
                                                    @endif
                                                </span>
                                            </td>
                                            <td class="px-2 py-4 text-xs leading-5 2xl:text-sm xl:px-4 whitespace-nowrap">{{$rentedBook->librarian->name}}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="flex flex-row">
                            <div class="inline-block w-full text-right py-[7px] mr-[100px] text-white">
                                ...
                                <button type="submit"
                                    ...
                                    disabled>
                                    <i class="fas fa-check mr-[7px]"></i> Vrati knjigu 
                                </button>
                            </div>
                        </div>
                    </div>
                </form>
            @elseif(count($rentedBooks) == 0 && isset($_GET['buttonSearchReturn']))
